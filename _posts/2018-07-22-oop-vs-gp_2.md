...
## 6. 推广的更多模型
现在我们的GCD算法可以处理整形和高斯整数了，但是GCD还可以处理更多的问题。例如一元多项式。

先来讨论多项式的情况。多项式我们可以使用std::vector来模拟。
```cpp
template <typename T>
class polynomial {
public:
    ...
    polynomial() : coefs_({0}) {} // f(x) = 0 for default
    // construct x^order
...
```
这是一个简单polynomial的申明，还有一些copy assign的接口和实现等，受限于篇幅，我们就不给出详细列出了。当然我们还需要一些operator的重载，同样也只给出申明。
```cpp
template <typename T>
...
inline static polynomial<T> operator% (
    polynomial<T> const& lhs, polynomial<T> const& rhs);
```
这里给出了多项式求商接口(operator/)，和求余接口(operator%)，处理多项式就跟处理整数一样了:
```cpp
// meta function for 
template <typename T>
inline constexpr bool is_polynomial_v = 
    std::conjunction_v<
        ...
        std::is_signed<safe_value_type_t<T>>
    >;

template <typename T>
auto remainder(T a, T b) ->
    std::enable_if_t<is_polynomial_v<T>, T> {
    return a % b;
}
```
当然有兴致，也可以实现一个简单的多项式带余除法。
```cpp
template <typename T>
inline static auto remainder(T const& a, T const& b) ->
    std::enable_if_t<is_poplynomial_v<T>, T> {
    auto quotient = T{};
    auto remainder = a;
...
```
长除法的思路很简单，把整个多项式除法分解为多轮简单的除法。停止条件是整除或者余式的阶小于除式的阶。一轮长除法循环中，商式的阶等于每次去被除式子与除式阶的差，而系数是被除式与除式最高次的系数的比值。被除式减去商式与除式之积得到的结果，就是下一轮的被除式，直到循环结束。

多项式的remainder可以正确工作以后，我们的原先抽象的GCD的通用外观，应该不需要修改任何代码就可以对多项式正确工作了。
